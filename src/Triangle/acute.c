...
void writeparts(mesh *m, behavior *b, int argc, char **argv)
{
  ...
  strcpy(partfilename, b->innodefilename);
...
/*****************************************************************************/
/*                                                                           */
/* returnMinAngle()   Finds min angle of a triangle for .part file           */
/*                                                                           */
/*                                                                           */
/*****************************************************************************/
REAL returnMinAngle(REAL p1x, REAL p1y,REAL p2x, REAL p2y, REAL p3x, REAL p3y){
	REAL angle_p1, angle_p2, angle_p3,a2,b2,c2;
	a2 = (p1x-p2x)*(p1x-p2x)+(p1y-p2y)*(p1y-p2y);
	b2 = (p1x-p3x)*(p1x-p3x)+(p1y-p3y)*(p1y-p3y);
	c2 = (p2x-p3x)*(p2x-p3x)+(p2y-p3y)*(p2y-p3y);
	angle_p1 = acos((a2 + b2 - c2)/(2.0*sqrt(a2)*sqrt(b2)))*180.0/PI;
	angle_p2 = acos((a2 + c2 - b2)/(2.0*sqrt(a2)*sqrt(c2)))*180.0/PI;
	angle_p3 = acos((c2 + b2 - a2)/(2.0*sqrt(c2)*sqrt(b2)))*180.0/PI;
...
}// end of returnMinAngle()

/*****************************************************************************/
/*                                                                           */
/* returnMaxAngle()   Finds max angle of a triangle for .part file           */
/*                                                                           */
/*                                                                           */
/*****************************************************************************/
...
void detailedHistogram(mesh *m, behavior *b)
{
	...
	int i, ii, j, k;
	REAL cossquaretableHist[89];
	REAL radconstHist, degconstHist;	
	int onedegree;
	int angletableHist[180];
 	FILE *outfile;
        char histfilename[FILENAMESIZE];
        strcpy(histfilename, b->innodefilename);
        histfilename[strlen(histfilename) - 5] = '\0';
        strcat(histfilename, ".1.histogram");	
	if (!b->quiet) {
		printf("Writing %s.\n", histfilename);
	}
	...
	radconst = PI / 18.0;
	degconst = 180.0 / PI;	
	radconstHist = PI / 180.0;
	degconstHist = 180.0 / PI;
	for (i = 0; i < 89; i++) {
		cossquaretableHist[i] = cos(radconstHist * (REAL) (i + 1));
	...
	printf("  Detailed Angle Histogram:\n");
	for (i = 0; i < 90; i++) {
		printf("    %3d - %3d degrees:  %8d    |    %3d - %3d degrees:  %8d\n",
		       i , i + 1, angletableHist[i],
		       i + 90, i + 91, angletableHist[i + 90]);
	}
	printf("\n");
	// write data to a file
 	for (i = 0; i < 180; i++) {
 		fprintf(outfile,"    %3d  %3d  %8d    \n",
		       i , i + 1, angletableHist[i]);
	}
	fclose(outfile);
}
